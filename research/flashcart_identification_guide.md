...
flashcarts from photographs. It is designed to be consumed by an LLM (e.g., Claude)
that assists users in identifying their carts and selecting the correct kernel.
...
## CRITICAL RULE: ALWAYS EXAMINE THE BACK OF THE CART FIRST

**DO NOT identify a flashcart from the front label.** DS flashcart labels are
deliberately misleading. Manufacturers put DEMON-family stickers on Ace3DS+
clone hardware. The front label is the LEAST reliable identifier.

**Before analyzing any label text, you MUST:**

1. Ask the user for a photo of the **back** of the cartridge.
2. Examine the **PCB (circuit board) color** visible through the shell.
3. Examine the **shell indent pattern** on the sides of the cart.

Only after checking these physical markers should you look at the front label.
**Physical markers override label text in every case.**

---

## 1. Identification Procedure

Given one or more photos of a flashcart, apply these steps **in this exact
order**. Do NOT skip to label analysis.
...
2. CHECK PCB COLOR (requires back-of-cart photo)
   ...
   -> Cannot determine: Ask user for a better photo of the back. If
      unavailable, proceed to step 3.

3. CHECK SHELL INDENT PATTERN (requires side view)
...
**If the user only provides a front photo:** Do NOT guess from the label.
Ask for a photo of the back of the cart before making an identification.
Labels like "Dual-Core," "SMART UPDATE," "Gold Pro," and "RTS Lite" appear
on both genuine DEMON carts AND on relabeled Ace3DS+ clones. You cannot
distinguish them without seeing the PCB.

---

## 2. PCB Color Reference (Most Reliable Single Indicator)
...
**Key rule:** If the PCB is red, it is almost certainly an Ace3DS+/R4iLS clone
regardless of what the front label says. This is the single most reliable visual
indicator.

---

## 3. Shell Indent Pattern (Second Most Reliable)
...
## 4. Confidence Levels for Image-Based Identification
...
## 6. Why Labels Cannot Be Trusted
...
for PCB color, shell geometry, and other physical markers.

### Front Label Analysis
...
#### Labels That Suggest Ace3DS+ / R4iLS Clone
...
#### Labels That Suggest Genuine DEMON Cart (But Verify with PCB/Shell)
...
**Critical caveat:** All of the DEMON-style labels above are now commonly
found on relabeled Ace3DS+ clones. You **must** verify with PCB color and shell indents.

#### Distinguishing r4isdhc.hk -- Clone vs DEMON
...
### Software-Based Identification (Boot Behavior)
...
## 7. The Major Hardware Families (Currently Sold)
...
### 7.2 DSTTi DEMON Clone Family (AKA "Timebomb Carts")
...
## 8. Kernel Quick Reference

### 8.1 Ace3DS+ / R4iLS Clone Family
...
## 9. Dangerous Misidentifications (Brick Risks)
...
## 10. Case Study: R4 SDHC Dual-Core PLUS (No URL)
...
### 12.2 Original DSTT / DSTTi (ndstt.com)
...
### 12.3 Original R4SDHC (r4sdhc.com)
